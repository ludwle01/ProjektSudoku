...
Die Funktion main() bildet den Einstiegspunkt des Programms. Sie bleibt bewusst kurz, ruft callStartMenu() auf und beendet das Programm nach dessen Rückkehr mit dem Rückgabewert 0. Die eigentliche Steuerung erfolgt über zwei Menüfunktionen. Im Startmenü wählt der User, ob ein Sudoku mit einer von drei Schwierigkeitsstufen erstellt oder aus inputSudoku.txt eingelesen werden soll. Ein eingelesenes Sudoku wird zunächst mit validate() geprüft. Ist es bereits vollständig, wird dies direkt ausgegeben. Andernfalls prüft checkSolvability() einmalig mithilfe des Donald-Knuth-Algorithmus, ob und wie oft das Sudoku lösbar ist.
Danach führt sudokuMenu() zu den weiteren Funktionen. Der User kann das Sudoku selbst lösen, einen der beiden Solver starten oder sich einen Motivationsspruch anzeigen lassen. Nach einer automatischen Lösung kann das Ergebnis gespeichert werden. getUserIntInput() prüft sämtliche Menüeingaben und fängt Buchstaben sowie Werte außerhalb des jeweiligen Bereichs ab. Beim Verlassen gibt returnToMenu() den dynamisch reservierten Speicher der Matrix frei.

\section{InteractiveSolver()}

Das interaktive Lösen selbst wird in der Funktion von playSudokuInteractive() gesteuert. Vor jedem Spielzug wird das aktuelle Sudoku zusammen mit einem Aktionsmenü ausgegeben. Der User kann das Spiel abbrechen, eine Zahl eintragen oder einen Hinweis anfordern. getUserInput() erwartet Zeile, Spalte und Zahl als drei ganze Zahlen. Falsche Formate und Werte außerhalb von 1 bis 9 werden abgefangen. Für den Zugriff auf die Matrix werden die eingegebenen Koordinaten anschließend auf die Indizes 0 bis 8 umgerechnet.
Ein Zug wird zuerst von isLegalMove() geprüft. Dabei muss das gewählte Feld leer sein und die Sudokuregeln dürfen nicht verletzt werden. Danach setzt isStillSolvable() den Wert nur testweise ein und prüft mit dem Donald-Knuth-Algorithmus, ob mindestens eine Lösung verbleibt. Erst wenn beide Prüfungen erfolgreich sind, wird die Zahl dauerhaft übernommen. Nach jedem Zug kontrolliert isSudokuFinished(), ob alle Felder belegt sind. In diesem Fall kann die Lösung gespeichert werden.
Für einen Hinweis wird eine Kopie des aktuellen Sudokus mit dem Backtracking-Solver gelöst. Aus dieser Kopie kann eine Zeile, ein 3×3-Block oder ein einzelnes Feld angezeigt werden, ohne den Spielstand zu verändern. Diese Ausgaben werden bewusst nicht direkt ins Sudoku eingetragen, um den Rätzelspaß nicht zu mindern.



\section{read()} \label{read}

Eine read() Methode zum Einlesen von Sudoku-Rätseln ist essenziell für die Funktionalität des Tools. Hierfür gibt der User sein Sudoku in die Textdatei inputSudoku.txt ein. Anschließend wird versucht, die Textdatei im Lesemodus zu öffnen. Ist das Öffnen erfolgreich, liest die Methode mithilfe einer while-Schleife das Sudoku ein. Hierbei werden kleine Fehler bei der Nutzereingabe ignoriert. Leerzeichen, senkrechte Striche, Bindestriche und leere Zeilen werden übersprungen. Bereits bei dem Einlesen wird eine grundlegende Validierung vorgenommen. Die Methode wirft eine Fehlermeldung, wenn in einer Zeile, die nicht leer ist, mehr oder weniger als 9 Zahlen vorkommen. Außerdem werden nur Eingaben von 0 bis 9 akzeptiert. Die Zahl 0 steht hierbei für ein leeres Feld. Auch bei zu vielen oder zu wenigen Zeilen wird eine Fehlermeldung ausgegeben. Handelt es sich bei dem eingegeben Zeichen um eine Zahl zwischen 0 und 9, wird durch die Substraktion des ASCII Wertes von 0 der korrekte Wert in der Matrix gespeichert. Zuletzt ruft die Funktion printMatrix() auf, um die eingelesene Matrix in der Konsole auszugeben.

\section{create()}

Alternativ kann man sich auch mithilfe von create() ein zufälliges, eindeutiges Sudoku generieren lassen. Hierfür werden zwei zufällige Zahlen in ein leeres Sudoku eingetragen. Diese stellen sicher, dass bei jedem Ausführen der Funktion unterschiedliche Sudokus generiert werden. Nachdem mithilfe der Funktion \textit{DonaldKnuth} auf Gültigkeit geprüft wurde und eine Lösung für das Sudoku erstellt wurde, werden dann je nach Schwierigekit mehr oder wengier Zahlen entfernt. Hierbei wird nach jeder entfernten Zahl geprüft, ob das Sudoku noch eindeutig lösbar ist, sonst wird eine andere Zahl entfernt.
...
Indem die Indizes der gleichen verschachtelten for-Schleifen getauscht werden, können die Spalten des Sudokus analog überprüft werden. Es wird mit der selben Logik vorgegangen. Hierbei repäsentiert jede Spalte in der Matrix \textit{spalte} eine Spalte in dem Sudoku.
...
Eine Funktion \textit{solveMatrix()} nutzt dann Rekursion und Backtracking, um eine Lösung zu finden. Hierbei werden bei jeder Null in \textit{coveredColumns} alle gültigen Spielzüge gezählt, wobei gezielt nach der Spalte mit den weigsten möglichen Spielzügen gesucht wird, um ein frühzeitiges Erkennen von Sackgassen zu ermöglichen. Wird eine Zeile gefunden, wird diese in \textit{solution} und \textit{coveredColumns} gespeichert. Falls \textit{coveredColumns} noch nicht ausschließlich mit Einsen gefüllt ist, aber trotzdem keine passende Regel vorhanden ist, wird der letzte Spielzug rückgängig gemacht und die Rekursion läuft weiter. Je nach Modus kann das Sudoku entweder normal gelöst werden oder über eine Funktion \textit{countSolutionsMatrix()} die Anzahl der Lösungen ermitteln.
...
\section{Tests}
Die automatisierten Tests befinden sich in testSudoku.c und werden über eine eigene main()-Funktion gestartet. Die Tests zielen besonders auf randfälle und falsche eingaben des Users ab. Die Hilfsfunktion pruefeTest() vergleicht für jeden Test das erhaltene mit dem erwarteten Ergebnis. Weitere Hilfsfunktionen erstellen und löschen die dynamischen Testmatrizen oder prüfen, ob ein gelöstes Sudoku vollständig ausgefüllt ist.
Insgesamt werden 17 Einzeltests ausgeführt. validate() wird mit einem gültigen Sudoku sowie mit doppelten Zahlen in Zeile, Spalte und 3×3-Block getestet. Für validate\_cell() werden ein erlaubter Wert und Konflikte in allen drei Bereichen geprüft. Beide Solver müssen eine Lösung finden, diese vollständig ausfüllen und ein gültiges Sudoku erzeugen. Beim Donald-Knuth-Algorithmus wird zusätzlich die eindeutige Lösbarkeit kontrolliert. createSudoku() muss ein gültiges und eindeutig lösbares Sudoku erstellen. Als Randfall wird außerdem geprüft, ob read() eine leere Eingabedatei korrekt abfängt. Eine vorhandene Datei wird dafür vorübergehend gesichert und anschließend wiederhergestellt.
Erwartete Fehlerausgaben der Funktionen selbst werden abhängig vom Betriebssystem nach NUL beziehungsweise /dev/null umgeleitet, dies dient der besseren Übersichtlichkeit. Sind alle Tests bestanden, beendet sich das Testprogramm mit 0, andernfalls mit 1. Da es eine eigene main()-Funktion besitzt, darf main.c beim Kompilieren der Tests nicht eingebunden werden. In der finalen Version wurden alle 17 Tests erfolgreich bestanden.
...
Die Implementierung des Sudoku-Tools konnte erfolgreich abgeschlossen und durch das Bestehen aller finalen Tests vollständig verifiziert werden. Mit der fertigen Anwendung steht nun ein vielseitiges Werkzeug zur Verfügung, das es dem Nutzer ermöglicht, Sudokus sowohl interaktiv über die Benutzeroberfläche zu lösen als auch programmgestützt valide Rätsel generieren zu lassen.
...
\item \textbf{Jari Hartmann:} Mainmethode, interactiveSolver(), Tests
\end{itemize}
