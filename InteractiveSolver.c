...
int getUserInput(
    ...
    if(matches != 3) {
        printf(
            "FEHLER! Bitte drei ganze Zahlen eingeben, "
        );
    ...
    if(
        *row < 1
        ...
        printf(
            "FEHLER! Zeile, Spalte und Zahl muessen "
        );
...
static void showHint(int **sudoku) {
    ...
    if(solution == NULL) {
        printf(
            "\nFehler: Fuer den Hinweis konnte kein "
        );
    ...
    if(solve(0, 0, solution) == 0) {
        printf(
            "\nFuer dieses Sudoku konnte keine "
    ...
    if(hintOption == 1) {
        printf(
            "Welche Zeile soll angezeigt werden? (1-9): "
        );
    ...
    //Einen vollstaendigen 3x3-Block anzeigen
        ...
        printf(
            "Welcher Block soll angezeigt werden?\n"
        );

        printf(
            "1 = oben links, 2 = oben Mitte, ..., "
            "9 = unten rechts: "
        );
    ...
    //Den Wert eines einzelnen Feldes anzeigen
        ...
        printf(
            "In welcher Zeile liegt das Feld? (1-9): "
        );

        int row = getUserIntInput(1, 9);

        printf(
            "In welcher Spalte liegt das Feld? (1-9): "
        );

        int column = getUserIntInput(1, 9);

        printf(
            "\nIn einer moeglichen Loesung steht im Feld "
        );
...
void playSudokuInteractive(int **sudoku) {
    ...
    printf(
        "(Setzt in Zeile 1, Spalte 2, die Zahl 4 ein)\n"
    );
    ...
    while(running) {
        ...
        if(inputStatus == 0) {
            ...
            if(
                ...
                printf(
                    "FEHLER! Der Zug ist nach den "
                );

                printf(
                    "Sudoku-Regeln nicht erlaubt!\n"
                );
                ...
                printf(
                    "SACKGASSE! Das Sudoku ist danach "
                );
